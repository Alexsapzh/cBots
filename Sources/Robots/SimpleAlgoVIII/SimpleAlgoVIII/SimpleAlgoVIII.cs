...
namespace cAlgo.Robots
{
    ...
    public class RobotForexMS : Robot
    {
        ...
        {
            // Execute each symbol 
            Execute(MarketSeries, Symbol);
            // chart symbol
            Execute(_series2, _symbol2);
            // additional symbol
        }
        ...
        private void TrailPositions(Symbol symbol)
        {
            ...
            foreach (Position position in positions)
            {
                ...
                {
                    if (bid - GetAveragePrice(TradeType.Buy, symbol) >= TraiStart * pipSize && bid - TraiStop * pipSize >= position.StopLoss || position.StopLoss == null)
                        ModifyPosition(position, bid - TraiStop * pipSize, position.TakeProfit);
                }
                else
                {
                    if (GetAveragePrice(TradeType.Sell, symbol) - ask >= TraiStart * pipSize && (ask + TraiStop * pipSize <= position.StopLoss || position.StopLoss == null))
                ...
                switch (GetPositionsSide(symbol))
                {
                    case 0:
                        if (symbol.Ask < FindLastPrice(TradeType.Buy, symbol) - pipstep * symbol.PipSize)
                        {
                            ...
                            {
                                TradeResult result = ExecuteMarketOrder(TradeType.Buy, symbol, newVolume, RobotLabel, null, TakeProfit);
                    ...
                    case 1:
                        if (symbol.Bid > FindLastPrice(TradeType.Sell, symbol) + pipstep * symbol.PipSize)
                        {
                            if (!(newVolume < lotStep))
                            {
                                TradeResult result = ExecuteMarketOrder(TradeType.Sell, symbol, newVolume, RobotLabel, null, TakeProfit);
                                if (!result.IsSuccessful)
                                    if (result.Error.Equals(ErrorCode.NoMoney) || result.Error.Equals(ErrorCode.BadVolume))
                                        Stop();
                            }
        ...
        private int GetStdIlanSignal(MarketSeries marketSeries)
        {
            ...
            if (marketSeries.Close[lastBarIndex] > marketSeries.Open[lastBarIndex] && marketSeries.Close[prevBarIndex] > marketSeries.Open[prevBarIndex])
                return 0;
