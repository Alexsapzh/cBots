...
namespace cAlgo
{
    ...
    public class ElConejo : Robot
    {
        ...
        [Parameter("HMA HTF Period", DefaultValue = 2, MinValue = 2, MaxValue = 200, Step = 1)]
        ...
        protected override void OnBar()
        {
            ...
            if (rsi.Result.LastValue < 40 && _macd.Histogram.LastValue < 0.0 && _macd.Signal.IsRising() && previoushmaslow > previoushmafast && hmaSignal.IsBullish && longPosition == null)
            {
                if (shortPosition != null)
                    ClosePosition(shortPosition);
                ExecuteMarketOrder(TradeType.Buy, Symbol, VolumeInUnits, label, StopLoss, TakeProfit);
            }
            else if (rsi.Result.LastValue > 60 && _macd.Histogram.LastValue > 0.0 && _macd.Signal.IsFalling() && previoushmaslow < previoushmafast && hmaSignal.IsBearish && shortPosition == null)
            {
