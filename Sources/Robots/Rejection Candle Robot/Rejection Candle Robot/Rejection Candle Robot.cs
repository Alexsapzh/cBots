...
namespace cAlgo
{
    ...
    public class RejectionCandleRobot : Robot
    {
        ...
        [Parameter("Medium Periods", DefaultValue = 21)]
        ...
        protected override void OnBar()
        {
            ...
            if (maFilter)
            {
                ...
                if (bullishRejection && fastMa.Result.Last(0) > mediumMa.Result.Last(0) && mediumMa.Result.Last(0) > slowMa.Result.Last(0) && fastMa.Result.Last(0) < Symbol.Ask)
                {
                    mDistance = (fastMa.Result.Last(0) - mediumMa.Result.Last(0)) * Math.Pow(10, Symbol.Digits - 1) >= maDistance;
                    if (mDistance)
                        isMaOk = true;
                }
                else if (bearishRejection && fastMa.Result.Last(0) < mediumMa.Result.Last(0) && mediumMa.Result.Last(0) < slowMa.Result.Last(0) && fastMa.Result.Last(0) > Symbol.Bid)
                {
                    mDistance = (mediumMa.Result.Last(0) - fastMa.Result.Last(0)) * Math.Pow(10, Symbol.Digits - 1) >= maDistance;
                    if (mDistance)
                        isMaOk = true;
                }
                else
