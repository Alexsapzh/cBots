...
namespace cAlgo.Robots
{
    ...
    public class ConvertedRobot : Robot
    {
        ...
        {
            Mq4Double val = 0;
            Mq4Double InitialPrice = 0;
            Mq4Double spread = 0;
            Mq4Double SellGoal = 0;
            Mq4Double BuyGoal = 0;
            Mq4Double ProfitTarget = 0;
            Mq4Double PipsLot = 0;
            Mq4Double SellGoalProfit = 0;
            Mq4Double BuyGoalProfit = 0;
            Mq4Double total = 0;
            Mq4Double profit = 0;
            Mq4Double cpt = 0;
            Mq4Double ticket = 0;
            ...
            if (INCREMENT < MarketInfo(Symbol.Code, MODE_STOPLEVEL) + spread)
                INCREMENT = 1 + MarketInfo(Symbol.Code, MODE_STOPLEVEL) + spread;
            if (MONEY_MANAGEMENT)
                LOTS = NormalizeDouble(AccountBalance() * AccountLeverage() / 1000000 * RISK_RATIO, 0) * MarketInfo(Symbol.Code, MODE_MINLOT);
            if (LOTS < MarketInfo(Symbol.Code, MODE_MINLOT))
            {
            ...
            for (cpt = 0; cpt < OrdersTotal(); cpt++)
            {
                ...
                if (OrderMagicNumber() == MAGIC && OrderSymbol() == Symbol.Code)
                {
                    ...
                    {
                        val = getPipValueFunc(OrderOpenPrice(), OrderType());
                        takeProfitFunc(val, OrderTicket());
                    }
                }
            }
            if (total < 1 && Enter && (!UseEntryTime || (UseEntryTime && Hour() == EntryTime)))
            {
                ...
                {
                    Mq4Print("Not enough free margin to begin");
                    return 0;
                }
                ...
                for (cpt = 1; cpt <= LEVELS; cpt++)
                {
                    OrderSend(Symbol.Code, OP_BUYSTOP, LOTS, InitialPrice + cpt * INCREMENT * Point, 2, SellGoal, BuyGoal, DoubleToStr(InitialPrice, MarketInfo(Symbol.Code, MODE_DIGITS)), MAGIC, 0);
                    OrderSend(Symbol.Code, OP_SELLSTOP, LOTS, InitialPrice - cpt * INCREMENT * Point, 2, BuyGoal + spread * Point, SellGoal + spread * Point, DoubleToStr(InitialPrice, MarketInfo(Symbol.Code, MODE_DIGITS)), MAGIC, 0);
                }
            }
            else
            {
                ...
                for (cpt = 0; cpt < total; cpt++)
                {
                    ...
                    if (OrderSymbol() == Symbol.Code && OrderMagicNumber() == MAGIC && StrToDouble(OrderComment()) == InitialPrice)
                    {
                        EndSessionFunc();
                        return 0;
                    }
                }
                if (UseProfitTarget && CheckProfitsFunc(LOTS, OP_SELL, true, InitialPrice) > ProfitTarget)
                {
                    EndSessionFunc();
                    return 0;
                }
                BuyGoalProfit = CheckProfitsFunc(LOTS, OP_BUY, false, InitialPrice);
                SellGoalProfit = CheckProfitsFunc(LOTS, OP_SELL, false, InitialPrice);

                if (BuyGoalProfit < ProfitTarget)
                {
                    for (cpt = LEVELS; cpt >= 1 && BuyGoalProfit < ProfitTarget; cpt--)
                    {
                        if (Ask <= (InitialPrice + (cpt * INCREMENT - MarketInfo(Symbol.Code, MODE_STOPLEVEL)) * Point))
                        {
                            ticket = OrderSend(Symbol.Code, OP_BUYSTOP, cpt * LOTS, InitialPrice + cpt * INCREMENT * Point, 2, SellGoal, BuyGoal, DoubleToStr(InitialPrice, MarketInfo(Symbol.Code, MODE_DIGITS)), MAGIC, 0);
                        }
                        if (ticket > 0)
                            BuyGoalProfit += LOTS * (BuyGoal - InitialPrice - cpt * INCREMENT * Point) / Point;
                    }
                }
                ...
                if (SellGoalProfit < ProfitTarget)
                {
                    ...
                    {
                        if (Bid >= (InitialPrice - (cpt * INCREMENT - MarketInfo(Symbol.Code, MODE_STOPLEVEL)) * Point))
                        {
                            ticket = OrderSend(Symbol.Code, OP_SELLSTOP, cpt * LOTS, InitialPrice - cpt * INCREMENT * Point, 2, BuyGoal + spread * Point, SellGoal + spread * Point, DoubleToStr(InitialPrice, MarketInfo(Symbol.Code, MODE_DIGITS)), MAGIC, 0);
                        }
                        if (ticket > 0)
                            SellGoalProfit += LOTS * (InitialPrice - cpt * INCREMENT * Point - SellGoal - spread * Point) / Point;
                    }
                }
            }

            Comment("mGRID EXPERT ADVISOR ver 2.0\n", "FX Acc ServerMq4:", AccountServer(), "\n", "Date: ", Month(), "-", Day(), "-", Year(),
            " ServerMq4 Time: ", Hour(), ":", Minute(), ":", Seconds(), "\n", "Minimum Lot Sizing: ", MarketInfo(Symbol.Code, MODE_MINLOT), "\n",
            "AccountMq4 Balance:  $", AccountBalance(), "\n", "Symbol: ", Symbol.Code, "\n", "Price:  ", NormalizeDouble(Bid, 4), "\n", "Pip Spread:  ",
            MarketInfo("EURUSD", MODE_SPREAD), "\n", "Increment=" + INCREMENT, "\n", "Lots:  ", LOTS, "\n", "Levels: " + LEVELS, "\n");
            return 0;
            return 0;
        }
        Mq4Double CheckProfitsFunc(Mq4Double LOTS, Mq4Double Goal, Mq4Double Current, Mq4Double InitialPrice)
        {
            Mq4Double cpt = 0;
            Mq4Double profit = 0;
            profit = 0;
            if (Current)
            {
                ...
                {
                    OrderSelect(cpt, SELECT_BY_POS, MODE_TRADES);
                    if (OrderSymbol() == Symbol.Code && StrToDouble(OrderComment()) == InitialPrice)
                    {
                        if (OrderType() == OP_BUY)
        ...
        Mq4Double EndSessionFunc()
        {
            ...
            for (cpt = 0; cpt < total; cpt++)
            {
                Sleep(3000);
                OrderSelect(cpt, SELECT_BY_POS, MODE_TRADES);
                if (OrderSymbol() == Symbol.Code && OrderType() > 1)
                    OrderDelete(OrderTicket());
                else if (OrderSymbol() == Symbol.Code && OrderType() == OP_BUY)
                    OrderClose(OrderTicket(), OrderLots(), Bid, 3);
                else if (OrderSymbol() == Symbol.Code && OrderType() == OP_SELL)
        ...
        Mq4Double getPipValueFunc(Mq4Double ord, Mq4Double dir)
        {
            Mq4Double val = 0;
        ...
        void takeProfitFunc(Mq4Double current_pips, Mq4Double ticket)
        {
                ...
                if (current_pips >= nextTP && current_pips < (nextTP + Target_Increment))
                {
                    if (OrderType() == 1)
                    {
                        if (OrderClose(ticket, MAX_LOTS, Ask, 3))
                            ...
                            Mq4Print("Error closing order : ", GetLastError());
                    }
                    else
                    {
        ...
        [Parameter("INCREMENT", DefaultValue = 35)]
        public int INCREMENT_parameter { get; set; }
        bool _INCREMENTGot;
        Mq4Double INCREMENT_backfield;
        Mq4Double INCREMENT
        {
            get
            {
                if (!_INCREMENTGot)
                    INCREMENT_backfield = INCREMENT_parameter;
                return INCREMENT_backfield;
            }
            set { INCREMENT_backfield = value; }
        }
        ...
        [Parameter("LEVELS", DefaultValue = 3)]
        public int LEVELS_parameter { get; set; }
        bool _LEVELSGot;
        Mq4Double LEVELS_backfield;
        Mq4Double LEVELS
        {
            get
            {
                if (!_LEVELSGot)
                    LEVELS_backfield = LEVELS_parameter;
                return LEVELS_backfield;
            }
            set { LEVELS_backfield = value; }
        }

        [Parameter("MAX_LOTS", DefaultValue = 99)]
        public double MAX_LOTS_parameter { get; set; }
        bool _MAX_LOTSGot;
        Mq4Double MAX_LOTS_backfield;
        Mq4Double MAX_LOTS
        {
            get
            {
                if (!_MAX_LOTSGot)
                    MAX_LOTS_backfield = MAX_LOTS_parameter;
                return MAX_LOTS_backfield;
            }
            set { MAX_LOTS_backfield = value; }
        }

        [Parameter("MAGIC", DefaultValue = 1803)]
        ...
        [Parameter("CONTINUE", DefaultValue = true)]
        public bool CONTINUE_parameter { get; set; }
        bool _CONTINUEGot;
        Mq4Double CONTINUE_backfield;
        Mq4Double CONTINUE
        {
            get
            {
                if (!_CONTINUEGot)
                    CONTINUE_backfield = CONTINUE_parameter;
                return CONTINUE_backfield;
            }
            set { CONTINUE_backfield = value; }
        }

        [Parameter("MONEY_MANAGEMENT", DefaultValue = false)]
        public bool MONEY_MANAGEMENT_parameter { get; set; }
        bool _MONEY_MANAGEMENTGot;
        Mq4Double MONEY_MANAGEMENT_backfield;
        Mq4Double MONEY_MANAGEMENT
        {
            get
            {
                if (!_MONEY_MANAGEMENTGot)
                    MONEY_MANAGEMENT_backfield = MONEY_MANAGEMENT_parameter;
                return MONEY_MANAGEMENT_backfield;
            }
            set { MONEY_MANAGEMENT_backfield = value; }
        }

        [Parameter("RISK_RATIO", DefaultValue = 2)]
        public int RISK_RATIO_parameter { get; set; }
        bool _RISK_RATIOGot;
        Mq4Double RISK_RATIO_backfield;
        Mq4Double RISK_RATIO
        {
            get
            {
                if (!_RISK_RATIOGot)
                    RISK_RATIO_backfield = RISK_RATIO_parameter;
                return RISK_RATIO_backfield;
            }
            set { RISK_RATIO_backfield = value; }
        }
        ...
        Mq4Double nextTP;
        Mq4Double Enter = true;
        Mq4Double EntryTime = 0;
        Mq4Double UseEntryTime = false;
        Mq4Double First_Target = 20;
        Mq4Double Target_Increment = 50;
        Mq4Double UsePartialProfitTarget = false;
        Mq4Double UseProfitTarget = false;
