...
namespace cAlgo
{
    ...
    public class LaTortugaV3SlowWithClose : Robot
    {

        [Parameter(DefaultValue = "La Tortuga V3 Slow")]
        ...
        protected override void OnBar()
        {
            ...
            if (previoushmaslow > previoushmafast && currenthmaslow <= currenthmafast && hmaSignal.IsBullish && longPosition == null)
            {
                if (shortPosition != null)
                    ClosePosition(shortPosition);
                fBuy();
            }
            else if (previoushmaslow < previoushmafast && currenthmaslow >= currenthmafast && hmaSignal.IsBearish && shortPosition == null)
            {
            ...
            foreach (var position in cBotPositions)
            {
                if (position.Label == cBotLabel && position.SymbolCode == Symbol.Code)
                {
                    if (position.TradeType == TradeType.Buy)
                        if (Symbol.Ask < FindLastPrice(TradeType.Buy) - PipStep * Symbol.TickSize)
                            ExecuteMarketOrder(TradeType.Buy, Symbol, Volume, cBotLabel, StopLoss, TakeProfit);
                    if (position.TradeType == TradeType.Sell)
                    {
                        if (Symbol.Bid > FindLastPrice(TradeType.Sell) + PipStep * Symbol.TickSize)
