...
namespace cAlgo
{
    ...
    public class JawsV1SGPipStepHTF1hr : Robot
    {
        ...
        protected override void OnBar()
        {
            ...
            if (MarketSeries.Open.LastValue > _emaFast.Result.LastValue && _emaFast.Result.IsRising() && MarketSeries.Close.Last(1) > MarketSeries.Close.Last(2) && _macd.Histogram.LastValue < 0.0 && _macd.Signal.IsRising())
            {
                fBuy();
            }
            else if (MarketSeries.Open.LastValue < _emaFast.Result.LastValue && _emaFast.Result.IsFalling() && MarketSeries.Close.Last(1) < MarketSeries.Close.Last(2) && _macd.Histogram.LastValue > 0.0 && _macd.Signal.IsFalling())
            {
                fSell();
            }
            {
