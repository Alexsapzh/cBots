...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class GannSwings_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
            Mq4Double i = 0;
        ...
        void OutSidePartFunc()
        {
            ...
            {
                DownTrendOutSideFunc();
            }

            return;
            return;
        }
        void nonOutSidePartFunc()
        {
        ...
        Mq4Double GetLastSwingFunc()
        {
            Mq4Double point = 0;
            point = 1;
            while (HighSwings[point] == 0.0 && LowSwings[point] == 0.0 && point < Bars)
                point++;

            return point;
            return 0;
        }
        void SetPrevSwingFunc()
        {
            Mq4Double LastPoint = 0;

            LastPoint = GetLastSwingFunc();
            HighSwings[0] = 0.0;
        ...
        Mq4Double isOutSideSwingBarFunc(Mq4Double shift)
        {
            Mq4Double res = false;
            res = false;
            ...
            if (res && ShowOutSideBars)
            {
                if (Close[shift] > Open[shift])
                    UpCloseOutSideBuffers[shift] = (High[shift] + Low[shift]) / 2.0;
                else
                    DownCloseOutSideBuffers[shift] = (High[shift] + Low[shift]) / 2.0;
            }
            return res;
            return true;
        }
        void SetSwingsFunc(Mq4Double shift)
        {
            return;
            return;
        }
        Mq4Double Mq4Start()
        {
            ...
            for (cnt = limit; cnt > 0; cnt--)
            {
                ...
                nonOutSide = !isOutSideSwingBarFunc(cnt);

                if (High[cnt] > SwingHigh)
                {
                    highCounter++;
                    SwingHigh = High[cnt];
                    HighsBuffer[cnt] = SwingHigh;
                ...
                TrendBuffer[cnt] = TrendBuffer[cnt + 1];
                if (nonOutSide)
                {
                    nonOutSidePartFunc();
                }
        ...
        [Parameter("kind", DefaultValue = 1)]
        public int kind_parameter { get; set; }
        bool _kindGot;
        Mq4Double kind_backfield;
        Mq4Double kind
        {
            get
            {
                if (!_kindGot)
                    kind_backfield = kind_parameter;
                return kind_backfield;
            }
            set { kind_backfield = value; }
        }

        [Parameter("ShowHiLo", DefaultValue = false)]
        public bool ShowHiLo_parameter { get; set; }
        bool _ShowHiLoGot;
        Mq4Double ShowHiLo_backfield;
        Mq4Double ShowHiLo
        {
            get
            {
                if (!_ShowHiLoGot)
                    ShowHiLo_backfield = ShowHiLo_parameter;
                return ShowHiLo_backfield;
            }
            set { ShowHiLo_backfield = value; }
        }

        [Parameter("ShowOutSideBars", DefaultValue = false)]
        public bool ShowOutSideBars_parameter { get; set; }
        bool _ShowOutSideBarsGot;
        Mq4Double ShowOutSideBars_backfield;
        Mq4Double ShowOutSideBars
        {
            get
            {
                if (!_ShowOutSideBarsGot)
                    ShowOutSideBars_backfield = ShowOutSideBars_parameter;
                return ShowOutSideBars_backfield;
            }
            set { ShowOutSideBars_backfield = value; }
        }

        [Parameter("MoveLastSwing", DefaultValue = false)]
        public bool MoveLastSwing_parameter { get; set; }
        bool _MoveLastSwingGot;
        Mq4Double MoveLastSwing_backfield;
        Mq4Double MoveLastSwing
        {
            get
            {
                if (!_MoveLastSwingGot)
                    MoveLastSwing_backfield = MoveLastSwing_parameter;
                return MoveLastSwing_backfield;
            }
            set { MoveLastSwing_backfield = value; }
        }



        [Output("HighSwings", Color = Colors.Yellow, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries HighSwings_AlgoOutputDataSeries { get; set; }
        [Output("LowSwings", Color = Colors.Yellow, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries LowSwings_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries HighsBuffer_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries LowsBuffer_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries UpCloseOutSideBuffers_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries DownCloseOutSideBuffers_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries TrendBuffer_AlgoOutputDataSeries { get; set; }


        Mq4Double cnt;
        Mq4Double highCounter;
        Mq4Double lowCounter;
        Mq4Double myBars;
        Mq4Double LastSwing;
        Mq4Double prevSwing;
        Mq4Double SwingLow;
        ...
        int indicator_color2 = Yellow;
        int indicator_color3 = Blue;
        int indicator_color4 = Red;
        int indicator_color5 = Aqua;
        int indicator_color6 = HotPink;
        ...
        private Mq4OutputDataSeries HighSwings;
        private Mq4OutputDataSeries LowSwings;
        private Mq4OutputDataSeries HighsBuffer;
        private Mq4OutputDataSeries LowsBuffer;
        private Mq4OutputDataSeries UpCloseOutSideBuffers;
        private Mq4OutputDataSeries DownCloseOutSideBuffers;
        private Mq4OutputDataSeries TrendBuffer;
        ...
        {
            if (HighSwings_AlgoOutputDataSeries == null)
                HighSwings_AlgoOutputDataSeries = CreateDataSeries();
            HighSwings = new Mq4OutputDataSeries(this, HighSwings_AlgoOutputDataSeries, ChartObjects, 0, 0, () => CreateDataSeries(), 1, Colors.Yellow);
            AllBuffers.Add(HighSwings);
            if (LowSwings_AlgoOutputDataSeries == null)
                LowSwings_AlgoOutputDataSeries = CreateDataSeries();
            LowSwings = new Mq4OutputDataSeries(this, LowSwings_AlgoOutputDataSeries, ChartObjects, 0, 1, () => CreateDataSeries(), 1, Colors.Yellow);
            AllBuffers.Add(LowSwings);
            if (HighsBuffer_AlgoOutputDataSeries == null)
                HighsBuffer_AlgoOutputDataSeries = CreateDataSeries();
            HighsBuffer = new Mq4OutputDataSeries(this, HighsBuffer_AlgoOutputDataSeries, ChartObjects, 12, 2, () => CreateDataSeries(), 1, Colors.Blue);
            AllBuffers.Add(HighsBuffer);
            if (LowsBuffer_AlgoOutputDataSeries == null)
                LowsBuffer_AlgoOutputDataSeries = CreateDataSeries();
            LowsBuffer = new Mq4OutputDataSeries(this, LowsBuffer_AlgoOutputDataSeries, ChartObjects, 12, 3, () => CreateDataSeries(), 1, Colors.Red);
            AllBuffers.Add(LowsBuffer);
            if (UpCloseOutSideBuffers_AlgoOutputDataSeries == null)
                UpCloseOutSideBuffers_AlgoOutputDataSeries = CreateDataSeries();
            UpCloseOutSideBuffers = new Mq4OutputDataSeries(this, UpCloseOutSideBuffers_AlgoOutputDataSeries, ChartObjects, 3, 4, () => CreateDataSeries(), 1, Colors.Aqua);
            AllBuffers.Add(UpCloseOutSideBuffers);
            if (DownCloseOutSideBuffers_AlgoOutputDataSeries == null)
                DownCloseOutSideBuffers_AlgoOutputDataSeries = CreateDataSeries();
            DownCloseOutSideBuffers = new Mq4OutputDataSeries(this, DownCloseOutSideBuffers_AlgoOutputDataSeries, ChartObjects, 3, 5, () => CreateDataSeries(), 1, Colors.HotPink);
            AllBuffers.Add(DownCloseOutSideBuffers);
            if (TrendBuffer_AlgoOutputDataSeries == null)
                TrendBuffer_AlgoOutputDataSeries = CreateDataSeries();
            TrendBuffer = new Mq4OutputDataSeries(this, TrendBuffer_AlgoOutputDataSeries, ChartObjects, 12, 6, () => CreateDataSeries(), 1, null);
            AllBuffers.Add(TrendBuffer);

            AllOutputDataSeries.Add(HighSwings_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(LowSwings_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(HighsBuffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(LowsBuffer_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(UpCloseOutSideBuffers_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(DownCloseOutSideBuffers_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(TrendBuffer_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                HighSwings.SetCurrentIndex(index);
                LowSwings.SetCurrentIndex(index);
                HighsBuffer.SetCurrentIndex(index);
                LowsBuffer.SetCurrentIndex(index);
                UpCloseOutSideBuffers.SetCurrentIndex(index);
                DownCloseOutSideBuffers.SetCurrentIndex(index);
                TrendBuffer.SetCurrentIndex(index);
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly GannSwings_Indicator _indicator;

            public Mq4OutputDataSeries(GannSwings_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = GannSwings_Indicator.GetArrowByCode(ArrowCode);
