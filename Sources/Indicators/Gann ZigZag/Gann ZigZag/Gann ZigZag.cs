...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class GannZIGZAG_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
        ...
        Mq4Double Mq4Start()
        {
            ...
            {
                if (cb == 0)
                    FileWrite(idFile, "---- èíäåêñ òåêóùåãî áàðà " + cb + " " + (Bars - 1 - cb) + " âðåìÿ " + TimeToStr(Time[cb]) + " áàðîâ íà ãðàôèêå " + Bars);
                ...
                if (GSv_sl[cb + 1] > 0 && lb != Bars - 1 - cb)
                {
                    if (draw_up != 0)
                    {
                        s_dn = 0;
                        if (cb == 0)
                            FileWrite(idFile, "  1.0 Áûë îòðèñîâàí ìàêñèìóì, ñ÷åò÷èê ìèíèìóìîâ îáíóëåí");
                    }
                    else
                    {
                        if (draw_dn != 0)
                        {
                            s_up = 0;
                            if (cb == 0)
                                FileWrite(idFile, "  2.0 Áûë îòðèñîâàí ìèíèìóì, ñ÷åò÷èê ìàêñèìóìîâ îáíóëåí");
                        }
                    }
                }
                if (lb != Bars - 1 - cb)
                {
                    ...
                    cur_l = 0;
                    if (cb == 0)
                        FileWrite(idFile, "  2.1 íîâûé áàð ôëàãè íàëè÷èÿ ìàêñèìóìà è ìèíèìóìà ñáðîøåíû");
                }
                if (cb > Bars - 2 - drawf || (High[cb] <= High[cb + 1] && Low[cb] >= Low[cb + 1]))
                {
                    if (cb == 0)
                        FileWrite(idFile, "  3.0 Áàð ëèáî 'âíóòðåííèé', ëèáî äî ïåðâîãî ýêñòðåìóìà");
                    continue;
                }
                if (draw_up != 0)
                {
                    if (cb == 0)
                        FileWrite(idFile, "  4.0 Èíäèêàòîð íàïðàâëåí ââåðõ");
                    ...
                    if (High[cb] > h)
                    {
                        ...
                        cur_h = 1;
                        if (cb == 0)
                            FileWrite(idFile, "  5.0 Äîñòèãíóò íîâûé ìàêñèìóì = " + h + " cur_h " + cur_h);
                    }
                    if (Low[cb] < l)
                    {
                        ...
                        l = Low[cb];
                        if (cb == 0)
                            FileWrite(idFile, "  6.0 Äîñòèãíóò íîâûé ìèíèìóì = " + l);

                        if (lb != Bars - 1 - cb || cur_l != 1)
                            s_dn++;
                        cur_l = 1;
                        if (cb == 0)
                            FileWrite(idFile, "  7.0 Áàð íîâûé, ñ÷åò÷èê ìèíèìóìîâ óâåëè÷åí " + s_dn + " cur_l " + cur_l);
                    }

                    if (s_up == s_dn)
                    {
                        if (cb == 0)
                            FileWrite(idFile, "  8.0 Ñ÷åò÷èêè ðàâíû");

                        if (cur_h == cur_l && cur_l == 1)
                        {
                            if (cb == 0)
                                FileWrite(idFile, "  9.0 Åñòü äâà ýêñòðåìóìà");
                            ...
                            if (Close[cb] <= Open[cb])
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  10.0 Ñâå÷à ìåäâåæüÿ, ëèíèÿ ââåðõ = " + draw_up + ", ëèíèÿ âíèç," + draw_dn + " fPoint_i = " + fPoint_i + " sPoint_i " + sPoint_i + " èíäèêàòîð = " + GSv_sl[cb]);
                            }
                            else
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  11.0 Ñâå÷à áû÷üÿ, ëèíèÿ ââåðõ = " + draw_up + ", ëèíèÿ âíèç," + draw_dn + " fPoint_i = " + fPoint_i + " sPoint_i " + sPoint_i + " èíäèêàòîð = " + GSv_sl[cb]);
                            }
                        }
                        else
                        {
                            ...
                            if (cur_h == 1)
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  12.0 Òîëüêî ìàêñèìóì, èíäèêàòîð = " + GSv_sl[cb] + " sPoint_i " + sPoint_i + " l " + l);
                            }
                            else
                            {
                                if (cur_l == 1)
                                {
                                    ...
                                    if (cb == 0)
                                        FileWrite(idFile, "  13.0 Òîëüêî ìèíèìóì, èíäèêàòîð = " + GSv_sl[cb] + " fPoint_i " + fPoint_i + " sPoint_i " + sPoint_i + " draw_up " + draw_up + " draw_dn " + draw_dn + " h " + h);
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (cb == 0)
                            FileWrite(idFile, "  14.0 Ñ÷åò÷èêè íå ðàâíû");
                        ...
                        if (cur_h == 1)
                        {
                            ...
                            l = Low[cb];
                            if (cb == 0)
                                FileWrite(idFile, "  15.0 Åñòü íîâûé ìàêñèìóì, èíäèêàòîð " + GSv_sl[cb] + " sPoint_i " + sPoint_i + " l " + l);
                        }
                    }
                }
                else
                {
                    ...
                    if (cb == 0)
                        FileWrite(idFile, "  16.0 Èíäèêàòîð íàïðàâëåí âíèç");
                    if (High[cb] > h)
                    {

                        h = High[cb];
                        if (cb == 0)
                            FileWrite(idFile, "  17.0 Äîñòèãíóò íîâûé ìàêñèìóì " + h);
                        if (lb != Bars - 1 - cb || cur_h != 1)
                            s_up++;
                        cur_h = 1;

                        if (cb == 0)
                            FileWrite(idFile, "  18.0 Íîâûé áàð, ñ÷åò÷èê ìàêñèìóìîâ óâåëè÷åí " + s_up + " cur_h " + cur_h + " h " + h);
                    }
                    if (Low[cb] < l)
                    {
                        ...
                        cur_l = 1;
                        if (cb == 0)
                            FileWrite(idFile, "  19.0 Äîñòèãíóò íîâûé ìèíèìóì " + l + " cur_l " + cur_l);
                    }

                    if (s_up == s_dn)
                    {
                        if (cb == 0)
                            FileWrite(idFile, "  20.0 Ñ÷åò÷èêè ðàâíû");

                        if (cur_h == cur_l && cur_l == 1)
                        {
                            if (cb == 0)
                                FileWrite(idFile, "  21.0 Åñòü äâà ýêñòðåìóìà");
                            ...
                            if (Close[cb] <= Open[cb])
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  22.0 Ñâå÷à ìåäâåæüÿ, èíäèêàòîð " + GSv_sl[cb] + " sPoint_i " + sPoint_i);
                            }
                            else
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  23.0 Ñâå÷à áû÷üÿ, èíäèêàòîð " + GSv_sl[cb] + " draw_up " + draw_up + " draw_dn " + draw_dn + " sPoint_i " + sPoint_i + " fPoint_i " + fPoint_i);
                            }
                        }
                        else
                        {
                            ...
                            if (cur_h == 1)
                            {
                                ...
                                if (cb == 0)
                                    FileWrite(idFile, "  24.0 Òîëüêî ìàêñèìóì, èíäèêàòîð " + GSv_sl[cb] + " draw_up " + draw_up + " draw_dn " + draw_dn + " sPoint_i " + sPoint_i + " fPoint_i " + fPoint_i + " l " + l);
                            }
                            else
                            {
                                if (cur_l == 1)
                                {
                                    ...
                                    if (cb == 0)
                                        FileWrite(idFile, "  25.0 Òîëüêî ìèíèìóì, èíäèêàòîð " + GSv_sl[cb] + " sPoint_i " + sPoint_i + " h " + h);
                                }
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (cb == 0)
                            FileWrite(idFile, "  26.0 Ñ÷åò÷èêè íå ðàâíû");
                        ...
                        if (cur_l == 1)
                        {
                            ...
                            h = High[cb];
                            if (cb == 0)
                                FileWrite(idFile, "  27.0 Äîñòèãíóò íîâûé ìèíèìóì, èíäèêàòîð " + GSv_sl[cb] + " sPoint_i " + sPoint_i + " h " + h);
                        }
                    }
                }
                if (lb != Bars - 1 - cb)
        ...
        [Parameter("GSv_range", DefaultValue = 2)]
        public int GSv_range_parameter { get; set; }
        bool _GSv_rangeGot;
        Mq4Double GSv_range_backfield;
        Mq4Double GSv_range
        {
            get
            {
                if (!_GSv_rangeGot)
                    GSv_range_backfield = GSv_range_parameter;
                return GSv_range_backfield;
            }
            set { GSv_range_backfield = value; }
        }



        [Output("GSv_sl", Color = Colors.DeepSkyBlue, PlotType = PlotType.Line)]
        public IndicatorDataSeries GSv_sl_AlgoOutputDataSeries { get; set; }
        ...
        Mq4Double draw_dn = 0;
        Mq4Double draw_up = 0;
        Mq4DoubleArray GSv_m = new Mq4DoubleArray();
        ...
        private Mq4OutputDataSeries GSv_sl;
        ...
        {
            if (GSv_sl_AlgoOutputDataSeries == null)
                GSv_sl_AlgoOutputDataSeries = CreateDataSeries();
            GSv_sl = new Mq4OutputDataSeries(this, GSv_sl_AlgoOutputDataSeries, ChartObjects, 1, 0, () => CreateDataSeries(), 1, Colors.DeepSkyBlue);
            AllBuffers.Add(GSv_sl);

            AllOutputDataSeries.Add(GSv_sl_AlgoOutputDataSeries);
...
//{
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly GannZIGZAG_Indicator _indicator;

            public Mq4OutputDataSeries(GannZIGZAG_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = GannZIGZAG_Indicator.GetArrowByCode(ArrowCode);
