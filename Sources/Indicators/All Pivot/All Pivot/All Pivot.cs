...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class AllPivots_v2_multi_Indicator : Indicator
    {
        void ObjDelFunc()
        {
            for (; num <= CountPeriods; num++)
            {
                ...
                ObjectDelete("H1[" + num + "]" + period_key);
                ObjectDelete("H2[" + num + "]" + period_key);
                ObjectDelete("H3[" + num + "]" + period_key);
                ObjectDelete("H4[" + num + "]" + period_key);
                ObjectDelete("L1[" + num + "]" + period_key);
                ObjectDelete("L2[" + num + "]" + period_key);
                ObjectDelete("L3[" + num + "]" + period_key);
                ObjectDelete("L4[" + num + "]" + period_key);
            }
            return;
        }
        void PlotLineFunc(Mq4String name, Mq4Double value, Mq4Double line_color, Mq4Double style)
        {
        ...
        Mq4Double Mq4Init()
        {
            if (TimePeriod == "H1" || TimePeriod == "60")
            {
                period = PERIOD_H1;
                period_key = "H1";
            }
        ...
        Mq4Double Mq4Start()
        {
            ...
            for (shift = CountPeriods - 1; shift >= 0; shift--)
            {
                ...
                pstyle = Style_pivots;
                mstyle = Style_middle;
                cstyle = Style_camarilla;
                num = shift;

                PlotLineFunc("PP[" + num + "]" + period_key, P, Aqua, pstyle);
                if (Plot_pivots)
                {

                    PlotLineFunc("R1[" + num + "]" + period_key, R1, Red, pstyle);
                    PlotLineFunc("R2[" + num + "]" + period_key, R2, Orange, pstyle);
                    PlotLineFunc("R3[" + num + "]" + period_key, R3, Gold, pstyle);

                    PlotLineFunc("S1[" + num + "]" + period_key, S1, MediumSeaGreen, pstyle);
                    PlotLineFunc("S2[" + num + "]" + period_key, S2, Lime, pstyle);
                    PlotLineFunc("S3[" + num + "]" + period_key, S3, GreenYellow, pstyle);
                }

                if (Plot_middle)
                {
                    PlotLineFunc("M0[" + num + "]" + period_key, M0, LightBlue, mstyle);
                    PlotLineFunc("M1[" + num + "]" + period_key, M1, DeepSkyBlue, mstyle);
                    PlotLineFunc("M2[" + num + "]" + period_key, M2, MediumSlateBlue, mstyle);
                    PlotLineFunc("M3[" + num + "]" + period_key, M3, Tomato, mstyle);
                    PlotLineFunc("M4[" + num + "]" + period_key, M4, Chocolate, mstyle);
                    PlotLineFunc("M5[" + num + "]" + period_key, M5, Pink, mstyle);
                }

                if (Plot_camarilla)
                {
                    PlotLineFunc("H4[" + num + "]" + period_key, H4, LightBlue, cstyle);
                    PlotLineFunc("H3[" + num + "]" + period_key, H3, DeepSkyBlue, cstyle);
                    PlotLineFunc("H2[" + num + "]" + period_key, H2, MediumSlateBlue, cstyle);
                    PlotLineFunc("H1[" + num + "]" + period_key, H1, RoyalBlue, cstyle);

                    PlotLineFunc("L1[" + num + "]" + period_key, L1, Chocolate, cstyle);
                    PlotLineFunc("L2[" + num + "]" + period_key, L2, Orange, cstyle);
                    PlotLineFunc("L3[" + num + "]" + period_key, L3, Goldenrod, cstyle);
                    PlotLineFunc("L4[" + num + "]" + period_key, L4, Yellow, cstyle);
                }
            }
            return 0;
            return 0;
        ...
        [Parameter("CountPeriods", DefaultValue = 10)]
        public int CountPeriods_parameter { get; set; }
        bool _CountPeriodsGot;
        Mq4Double CountPeriods_backfield;
        Mq4Double CountPeriods
        {
            get
            {
                if (!_CountPeriodsGot)
                    CountPeriods_backfield = CountPeriods_parameter;
                return CountPeriods_backfield;
            }
            set { CountPeriods_backfield = value; }
        }

        [Parameter("TimePeriod", DefaultValue = "D1")]
        public string TimePeriod_parameter { get; set; }
        bool _TimePeriodGot;
        Mq4String TimePeriod_backfield;
        Mq4String TimePeriod
        {
            get
            {
                if (!_TimePeriodGot)
                    TimePeriod_backfield = TimePeriod_parameter;
                return TimePeriod_backfield;
            }
            set { TimePeriod_backfield = value; }
        }

        [Parameter("Plot_pivots", DefaultValue = true)]
        public bool Plot_pivots_parameter { get; set; }
        bool _Plot_pivotsGot;
        Mq4Double Plot_pivots_backfield;
        Mq4Double Plot_pivots
        {
            get
            {
                if (!_Plot_pivotsGot)
                    Plot_pivots_backfield = Plot_pivots_parameter;
                return Plot_pivots_backfield;
            }
            set { Plot_pivots_backfield = value; }
        }

        [Parameter("Plot_middle", DefaultValue = false)]
        public bool Plot_middle_parameter { get; set; }
        bool _Plot_middleGot;
        Mq4Double Plot_middle_backfield;
        Mq4Double Plot_middle
        {
            get
            {
                if (!_Plot_middleGot)
                    Plot_middle_backfield = Plot_middle_parameter;
                return Plot_middle_backfield;
            }
            set { Plot_middle_backfield = value; }
        }

        [Parameter("Plot_camarilla", DefaultValue = false)]
        public bool Plot_camarilla_parameter { get; set; }
        bool _Plot_camarillaGot;
        Mq4Double Plot_camarilla_backfield;
        Mq4Double Plot_camarilla
        {
            get
            {
                if (!_Plot_camarillaGot)
                    Plot_camarilla_backfield = Plot_camarilla_parameter;
                return Plot_camarilla_backfield;
            }
            set { Plot_camarilla_backfield = value; }
        }

        [Parameter("LineWidth", DefaultValue = 1)]
        public int LineWidth_parameter { get; set; }
        bool _LineWidthGot;
        Mq4Double LineWidth_backfield;
        Mq4Double LineWidth
        {
            get
            {
                if (!_LineWidthGot)
                    LineWidth_backfield = LineWidth_parameter;
                return LineWidth_backfield;
            }
            set { LineWidth_backfield = value; }
        }

        [Parameter("Style_Description", DefaultValue = "//STYLE_SOLID=0, STYLE_DASH=1, STYLE_DOT=2, STYLE_DASHDOT=3, STYLE_DASHDOTDOT=4")]
        public string Style_Description_parameter { get; set; }
        bool _Style_DescriptionGot;
        Mq4String Style_Description_backfield;
        Mq4String Style_Description
        {
            get
            {
                if (!_Style_DescriptionGot)
                    Style_Description_backfield = Style_Description_parameter;
                return Style_Description_backfield;
            }
            set { Style_Description_backfield = value; }
        }

        [Parameter("Style_pivots", DefaultValue = 4)]
        public int Style_pivots_parameter { get; set; }
        bool _Style_pivotsGot;
        Mq4Double Style_pivots_backfield;
        Mq4Double Style_pivots
        {
            get
            {
                if (!_Style_pivotsGot)
                    Style_pivots_backfield = Style_pivots_parameter;
                return Style_pivots_backfield;
            }
            set { Style_pivots_backfield = value; }
        }

        [Parameter("Style_middle", DefaultValue = 3)]
        public int Style_middle_parameter { get; set; }
        bool _Style_middleGot;
        Mq4Double Style_middle_backfield;
        Mq4Double Style_middle
        {
            get
            {
                if (!_Style_middleGot)
                    Style_middle_backfield = Style_middle_parameter;
                return Style_middle_backfield;
            }
            set { Style_middle_backfield = value; }
        }

        [Parameter("Style_camarilla", DefaultValue = 1)]
        public int Style_camarilla_parameter { get; set; }
        bool _Style_camarillaGot;
        Mq4Double Style_camarilla_backfield;
        Mq4Double Style_camarilla
        {
            get
            {
                if (!_Style_camarillaGot)
                    Style_camarilla_backfield = Style_camarilla_parameter;
                return Style_camarilla_backfield;
            }
            set { Style_camarilla_backfield = value; }
        }
        ...
        int indicator_color1 = LightBlue;
        int indicator_color2 = Tomato;
        int indicator_color3 = Aqua;
        int indicator_color4 = Orange;
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly AllPivots_v2_multi_Indicator _indicator;

            public Mq4OutputDataSeries(AllPivots_v2_multi_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = AllPivots_v2_multi_Indicator.GetArrowByCode(ArrowCode);
