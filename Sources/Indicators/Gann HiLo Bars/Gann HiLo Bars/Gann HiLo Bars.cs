...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class GannHiLobars_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
            IndicatorBuffers(3);
        ...
        Mq4Double Mq4Start()
        {
            ...
            {
                Hlv[i] = Hlv[i + 1];
                if (Close[i] > iMA(Symbol.Code, 0, Lb, 0, MODE_SMA, PRICE_HIGH, i + 1))
                    Hlv[i] = 1;
                if (Close[i] < iMA(Symbol.Code, 0, Lb, 0, MODE_SMA, PRICE_LOW, i + 1))
                    Hlv[i] = -1;
        ...
        [Parameter("Lb", DefaultValue = 10)]
        public int Lb_parameter { get; set; }
        bool _LbGot;
        Mq4Double Lb_backfield;
        Mq4Double Lb
        {
            get
            {
                if (!_LbGot)
                    Lb_backfield = Lb_parameter;
                return Lb_backfield;
            }
            set { Lb_backfield = value; }
        }



        public IndicatorDataSeries hi_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries lo_AlgoOutputDataSeries { get; set; }
        public IndicatorDataSeries Hlv_AlgoOutputDataSeries { get; set; }
        ...
        private Mq4OutputDataSeries hi;
        private Mq4OutputDataSeries lo;
        private Mq4OutputDataSeries Hlv;
        ...
        {
            if (hi_AlgoOutputDataSeries == null)
                hi_AlgoOutputDataSeries = CreateDataSeries();
            hi = new Mq4OutputDataSeries(this, hi_AlgoOutputDataSeries, ChartObjects, 2, 0, () => CreateDataSeries(), 1, Colors.Blue);
            AllBuffers.Add(hi);
            if (lo_AlgoOutputDataSeries == null)
                lo_AlgoOutputDataSeries = CreateDataSeries();
            lo = new Mq4OutputDataSeries(this, lo_AlgoOutputDataSeries, ChartObjects, 2, 1, () => CreateDataSeries(), 1, Colors.Red);
            AllBuffers.Add(lo);
            if (Hlv_AlgoOutputDataSeries == null)
                Hlv_AlgoOutputDataSeries = CreateDataSeries();
            Hlv = new Mq4OutputDataSeries(this, Hlv_AlgoOutputDataSeries, ChartObjects, 12, 2, () => CreateDataSeries(), 1, null);
            AllBuffers.Add(Hlv);

            AllOutputDataSeries.Add(hi_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(lo_AlgoOutputDataSeries);
            AllOutputDataSeries.Add(Hlv_AlgoOutputDataSeries);
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                hi.SetCurrentIndex(index);
                lo.SetCurrentIndex(index);
                Hlv.SetCurrentIndex(index);
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly GannHiLobars_Indicator _indicator;

            public Mq4OutputDataSeries(GannHiLobars_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = GannHiLobars_Indicator.GetArrowByCode(ArrowCode);
